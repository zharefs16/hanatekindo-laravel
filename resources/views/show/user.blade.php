...
<div class="col-xl-12 col-lg-7 col-md-9">
    <div class="card mx-4">
        <div class="card-body p-4">
            ...
            <form action="/user/update" method="post" class="row g-3 needs-validation" id="form" enctype="multipart/form-data" novalidate="">
                <!-- @method('put') -->
                @csrf
                
                <input type="text" name="user_id" hidden value="{{ $users['user_id'] }}">
                <div class="col-12" id="perorangan">
                    <div class="card mb-4">
                        <div class="card-header"><strong>Form</strong> <small>User</small></div>
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-12"><label class="form-label" for="user_fullname">Full Name</label><input class="form-control @error('name') is-invalid @enderror" id="user_fullname" type="text" name="user_fullname" required value="{{ $users['user_fullname'] }}">
                                ...
                                <div class="col-md-12"><label class="form-label" for="user_email">Email</label>
                                    <div class="input-group has-validation"><span class="input-group-text" id="inputGroupPrepend">@</span><input class="form-control @error('user_email') is-invalid @enderror" id="user_email" type="text" aria-describedby="inputGroupPrepend" name="user_email" required value="{{ $users['user_email'] }}">
                                        <div class="invalid-feedback">Please provide Email.</div>
                                        @error('user_email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                </div>
                                <div class="col-md-12"><label class="form-label" for="password">Password</label><input class="form-control @error('name') is-invalid @enderror" id="password" type="text" name="password" value="">
                                <div class="invalid-feedback">Please provide Password.</div>
                                @error('password')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-12"><label class="form-label" for="confirm_password">Konfirmasi password</label><input class="form-control @error('name') is-invalid @enderror" id="confirm_password" type="text" name="confirm_password" value="">
                                <div class="invalid-feedback">Please provide Password.</div>
